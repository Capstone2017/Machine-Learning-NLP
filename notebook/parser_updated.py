...
def download_file(download_url,name):
	...
	#file_.close()
    urllib.urlretrieve(download_url,name)
print("Completed")
...
def download_html():
	for curr in range(1,1692):
		#response = urllib.urlopen("https://e-foia.uspto.gov/Foia/DispatchBPAIServlet?SearchRng=decDt&docTextSearch=&page=60&d-3995525-p="+ str(curr)+ "&txtInput_EndDate=09%2F21%2F2016&SearchId=&Objtype=ser&txtInput_StartDate=09%2F01%2F1951")
		#webContent = response.read()

		#f = open("html/" + str(curr) + ".html", 'w')
		#f.write( webContent)
		#f.close()
            urllib.urlretrieve("https://e-foia.uspto.gov/Foia/DispatchBPAIServlet?SearchRng=decDt&docTextSearch=&page=60&d-3995525-p="+ str(curr)+ "&txtInput_EndDate=09%2F21%2F2016&SearchId=&Objtype=ser&txtInput_StartDate=09%2F01%2F1951","html/" + str(curr) + ".html")
...
def main(csv_name,start_page,end_page, start_row= None):
	with open(csv_name, 'a') as csvfile:
		...
		for curr in range(start_page,end_page):
			...
			soup = BeautifulSoup(open("html/" + str(curr) + ".html"), "lxml")
...
main('uspto_data_3_2.csv', 2, 3, 43)
